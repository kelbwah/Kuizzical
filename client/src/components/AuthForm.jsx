...
const AuthForm = () => { 
    ...
    return (
        ...
        >
            <form onSubmit={ (e) => isLoadingSubmission !== true ? submitData(e) : e.preventDefault() } ref={transitionRef} className='flex flex-col gap-2 items-center fade-in-faster lg:text-base md:text-base text-sm'>
                ...
                <button type='submit' className='w-full rounded-md font-bold text-gray-600 px-2 py-3 bg-yellow-300 hover:bg-yellow-200 active:bg-yellow-100 focus:outline-none ease-in-out duration-200 mb-4'>{isLoadingSubmission === false ? `${isLoginOrRegister}` : <Spinner />}</button>
                <p onClick={() => setIsLoginOrRegister((prev) => prev === 'Register' ? 'Login' : 'Register')} className='font-semibold text-gray-100 underline text-xs cursor-pointer hover:text-yellow-300 active:text-yellow-200 ease-in-out duration-200'>{isLoginOrRegister === 'Register' ? "Already have an account? Press here to login" : "Don't have an account? Press here to register"}</p>
            </form>
        </CSSTransition>
    );
};
